  ...
  return (
    ...
    >
      ...
      {showSummarySkeleton ? (
        ...
        >
          {statCards.map(
            ({ label, key, icon: Icon, iconBg, iconColor, accent }, index) => (
              <motion.div
                key={key}
                whileHover={{ y: -2 }}
                whileTap={{ scale: 0.99 }}
                layout
                className="relative overflow-hidden rounded-2xl border border-gray-200 bg-white p-6 shadow-sm transition-shadow hover:shadow-md dark:border-gray-800 dark:bg-gray-900"
              >
